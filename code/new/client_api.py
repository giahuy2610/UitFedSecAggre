...
import flwr as fl
import cv2
from FLclientHE import MalwarelientHE
import os
import numpy as np
import argparse
...
class ClientApi():
    ...
    def launch_fl_session(self, NoClient: str):
        ...
        fl.client.start_numpy_client(server_address=self.fl_server_address, client=MalwarelientHE(self.model_architecture  ,X_train, y_train, X_test, y_test, NoClient, self.clt_local_epochs))
...
# parser.add_argument('client_id', type=int, help='ID của client')
...
control.launch_fl_session(get_random_string(8))
